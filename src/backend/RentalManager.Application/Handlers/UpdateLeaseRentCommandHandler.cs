...
public class UpdateLeaseRentCommandHandler : IRequestHandler<UpdateLeaseRentCommand, LeaseDto>
{
    private readonly IApplicationDbContext _context;

    public UpdateLeaseRentCommandHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task<LeaseDto> Handle(UpdateLeaseRentCommand request, CancellationToken cancellationToken)
    {
        ...
        var newMonthlyRent = Money.Create(request.RentData.MonthlyRent, request.RentData.Currency);
        lease.UpdateRent(newMonthlyRent);
