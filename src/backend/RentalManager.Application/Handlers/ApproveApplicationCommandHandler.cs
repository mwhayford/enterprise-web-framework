...
public class ApproveApplicationCommandHandler : IRequestHandler<ApproveApplicationCommand, PropertyApplicationDto>
{
    ...
    public async Task<PropertyApplicationDto> Handle(ApproveApplicationCommand request, CancellationToken cancellationToken)
    {
        ...
        application.Approve(userId, request.DecisionNotes);

        await _context.SaveChangesAsync(cancellationToken);

        // Schedule background job to send approval email
        BackgroundJob.Enqueue<IApplicationNotificationJobs>(x =>
            x.SendApplicationStatusUpdateEmailAsync(application.Id, "Approved"));
