...
public interface ISearchService
{
    ...
    Task IndexDocumentAsync<T>(T document, string index, string? id = null)
        where T : class;

    Task IndexDocumentsAsync<T>(IEnumerable<T> documents, string index)
        where T : class;
    ...
    Task UpdateDocumentAsync<T>(T document, string index, string id)
        where T : class;
}
